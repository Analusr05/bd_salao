...
const AgendaController = {
  create: async (req, res) => {
    try {
      ...
      const agenda = await AgendaDAO.createAgendamento(
        horario,
        data,
        status,
        cliente_nome,
      ...
      res
        .status(201)
        .json({ message: "Agendamento criado com sucesso", data: agenda });
    } catch (error) {
      res
        .status(500)
  ...
  findById: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Erro ao buscar agendamento", details: error.message });
    }
  },
  ...
  update: async (req, res) => {
    try {
      ...
      const agenda = await AgendaDAO.update(id, {
        horario,
        data,
        status,
        cliente_nome,
  ...
  findByFuncionario: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: "Erro ao buscar agendamentos por funcionário",
          details: error.message,
        });
    }
  },
  ...
  getAgendaCompleta: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: "Erro ao buscar agenda completa",
          details: error.message,
        });
    }
  },
