...
const ProcedimentoController = {
  create: async (req, res) => {
    try {
      ...
      const procedimento = await ProcedimentoDAO.create({
        nome,
        preco,
        duracao,
      });
      ...
      res
        .status(500)
        .json({ error: "Erro ao criar procedimento", details: error.message });
    }
  },
  ...
  findById: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Erro ao buscar procedimento", details: error.message });
    }
  },
  ...
  update: async (req, res) => {
    try {
      ...
      const procedimento = await ProcedimentoDAO.update(id, {
        nome,
        preco,
        duracao,
      });
